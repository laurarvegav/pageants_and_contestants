...
RSpec.describe 'Pageants Contest', type: :feature do
 describe 'As a visitor' do
  before do
    ...
    @contestant2 = @pageant2.contestants.create!(name: "United States", years_of_experience: 78, has_representative: true)
    ...
    @contestant5 = @pageant2.contestants.create!(name: "Peru", years_of_experience: 75, has_representative: true)
  end

  #User Story 5, Parent Children Index
  it 'has a list of the associated childrens attributes' do
    visit "/pageants/#{@pageant2.id}/contestants"
  ...
  it "takes me back to the Contest Page where I see all of the Pageant's Contestants in alphabetical order" do
    ...
    expect(current_path).to eq("/pageants/#{@pageant2.id}/contestants")
    expect(@contestant1.name).to appear_before(@contestant5.name)
    expect(@contestant5.name).to appear_before(@contestant2.name)
  end

  # User Story 21, Display Records Over a Given Threshold 
  it "shows a form that allows me to input a number value and a submit button" do
    visit "/pageants/#{@pageant3.id}/contestants"
  ...
  it "brings me back to the current index page with only the records that meet that threshold shown." do
    ...
    expect(current_path).to eq("/pageants/#{@pageant3.id}/contestants")
